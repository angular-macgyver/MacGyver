...
angular.module("Mac").directive("macTime", [
  "$filter", "$timeout", "util", "keys", function($filter, $timeout, util, keys) {
    return {
      ...
      replace: true,
      template: "<div class=\"date-time\"><i class=\"mac-icons mac-icon-time\"></i><input type=\"text\" mac-placeholder=\"placeholder\" maxlength=\"8\" ng-model=\"model\" ng-disabled=\"disabled\" ng-click=\"updateInput()\" mac-keydown=\"keydownEvent($event)\" mac-keyup=\"keyupEvent($event)\"/></div>",
      compile: function(element, attrs) {
        var defaults, opts;
        defaults = {
          placeholder: "--:--",
