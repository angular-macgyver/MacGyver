...
angular.module("Mac").factory("modal", [
  ...
  "$rootScope", "$parse", "modal", "util", "keys", function($rootScope, $parse, modal, util, keys) {
    return {
      restrict: "E",
      template: "<div class=\"modal-overlay hide\"><div class=\"modal\"><a ng-click=\"closeModal($event)\" class=\"close close-modal\"></a><div ng-transclude=\"ng-transclude\"></div></div></div>",
      replace: true,
      transclude: true,
...
angular.module("Mac").directive("macTime", [
  "util", "keys", "$filter", function(util, keys, $filter) {
    return {
      ...
      replace: true,
      template: "<div class=\"date-time\"><i class=\"mac-icons mac-icon-time\"></i><input type=\"text\" placeholder=\"--:--\" maxlength=\"8\" ng-model=\"model\" ng-disabled=\"disabled\"/></div>",
      compile: function(element, attrs) {
        var defaults, inputElement, opts;
