...
angular.module("Mac").directive("macTagAutocomplete", [
  "$parse", "$http", "keys", function($parse, $http, key) {
    return {
      ...
      template: "<div class=\"mac-tag-autocomplete\"><ul class=\"tag-list\"><li ng-repeat=\"tag in tags\" class=\"tag label\"><div ng-click=\"removeTag(tag)\" class=\"tag-close\">X</div><span class=\"tag-label\"></span></li><li class=\"tag input-tag\"><mac-autocomplete mac-autocomplete-url=\"autocompleteUrl\" mac-autocomplete-on-select=\"onSelect(selected)\" mac-autocomplete-on-success=\"onSuccess(data)\" mac-autocomplete-on-key-down=\"onKeyDown(event, value)\" mac-autocomplete-clear-on-select=\"true\" class=\"text-input\"></mac-autocomplete></li></ul></div>",
      replace: true,
