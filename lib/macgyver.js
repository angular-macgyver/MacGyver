...
module.directive("macFocusOnEvent", function() {
  return function(scope, element, attributes) {
    return scope.$on(attributes.geFocusOnEvent, function() {
      return setTimeout((function() {
        var x, y;
        x = window.scrollX;
        y = window.scrollY;
...
angular.module("Mac").directive("macTable", [
  "$rootScope", "$compile", "$filter", "util", function($rootScope, $compile, $filter, util) {
    return {
      ...
      replace: true,
      transclude: true,
      template: "<div ng-style=\"getTableCss()\" class=\"mac-table-container mac-table\"><div ng-hide=\"!opts.showLoader || displayRows\" ng-cloak=\"ng-cloak\" class=\"mac-table-loader\"><mac-spinner mac-spinner-z-index=\"0\"></mac-spinner></div><div ng-show=\"opts.hasHeader\" class=\"mac-table-header\"><div class=\"mac-table-row\"></div></div><div class=\"mac-table-body-wrapper\"><div ng-show=\"opts.lockFirstColumn\" class=\"mac-title-column\"><div class=\"mac-table-row\"></div></div><div ng-style=\"getBodyBlockCss()\" class=\"mac-table-body\"></div><div class=\"mac-table-body-height\"></div></div><div class=\"mac-table-footer\"><div ng-show=\"opts.hasFooter\" class=\"mac-table-row custom-footer-row\"></div><div ng-show=\"opts.hasTotalFooter\" ng-style=\"getBodyBlockCss('total-footer')\" class=\"mac-table-row total-footer-row\"></div></div><div ng-transclude=\"ng-transclude\" ng-cloak=\"ng-cloak\" class=\"mac-table-transclude\"></div></div>",
...
angular.module("Mac").directive("macTagAutocomplete", [
  "$parse", "$http", "keys", function($parse, $http, key) {
    return {
      ...
      template: "<div class=\"mac-tag-autocomplete\"><ul class=\"tag-list\"><li ng-repeat=\"tag in tags\" class=\"tag label\"><div ng-click=\"removeTag(tag)\" class=\"tag-close\">&times;</div><span class=\"tag-label\"></span></li><li class=\"tag input-tag\"><span ng-show=\"disabled\"><input type=\"text\" ng-model=\"textInput\" class=\"text-input no-complete\"/></span><span ng-hide=\"disabled\"><mac-autocomplete mac-autocomplete-url=\"autocompleteUrl\" mac-autocomplete-on-select=\"onSelect(selected)\" mac-autocomplete-on-success=\"onSuccess(data)\" mac-autocomplete-on-key-down=\"onKeyDown(event, value)\" mac-autocomplete-clear-on-select=\"true\" class=\"text-input\"></mac-autocomplete></span></li></ul></div>",
      replace: true,
