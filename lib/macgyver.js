  ...
  "$http", "$parse", function($http, $parse) {
    return {
      ...
      compile: function(element, attrs) {
        var clearOnSelect, delay, labelKey, queryKey, valueKey;
  ...
  function() {
    return {
      ...
      link: function(scope, element, attributes) {
        ...
        disabled = attributes.macUploadDisabled === "true";
        ...
        if (disableOn != null) {
          scope.$on(disableOn, function() {
            return element.fileupload("disable");
          });
        }
        if (enableOn != null) {
...
angular.module("Mac").directive("macTable", [
  "$compile", "$filter", "util", function($compile, $filter, util) {
    return {
      ...
      replace: true,
      transclude: true,
      template: "<div ng-style=\"getTableCss()\" class=\"mac-table-container mac-table\"><div ng-hide=\"!opts.showLoader || !loading\" ng-cloak=\"ng-cloak\" class=\"mac-table-loader\"><mac-spinner mac-spinner-z-index=\"0\"></mac-spinner></div><div ng-show=\"opts.hasHeader\" class=\"mac-table-header\"><div class=\"mac-table-row\"></div></div><div class=\"mac-table-body-wrapper\"><div ng-show=\"opts.lockFirstColumn\" class=\"mac-title-column\"><div class=\"mac-table-row\"></div></div><div ng-style=\"getBodyBlockCss()\" class=\"mac-table-body\"></div><div class=\"mac-table-body-height\"></div></div><div class=\"mac-table-footer\"><div ng-show=\"opts.hasFooter\" class=\"mac-table-row custom-footer-row\"></div><div ng-show=\"opts.hasTotalFooter\" ng-style=\"getBodyBlockCss('total-footer')\" class=\"mac-table-row total-footer-row\"></div></div><div ng-transclude=\"ng-transclude\" ng-cloak=\"ng-cloak\" class=\"mac-table-transclude\"></div></div>",
      compile: function(element, attrs, transclude) {
        ...
        return function($scope, element, attrs) {
          ...
          $scope.getTableCss = function() {
            var dataLength, elementHeight, paddings, totalRows, width, _ref;
            dataLength = ((_ref = $scope.data) != null ? _ref.length : void 0) || 0;
            totalRows = dataLength === 0 || !opts.autoHeight ? opts.numDisplayRows : Math.min(dataLength, opts.numDisplayRows);
          ...
          $scope.calculateBodyDimension = function() {
            ...
            return setTimeout((function() {
              var dataLength, numRows, wrapperHeight, _ref;
              dataLength = ((_ref = $scope.data) != null ? _ref.length : void 0) || 0;
...
angular.module("Mac").directive("macTagAutocomplete", [
  "$parse", "$http", "keys", function($parse, $http, key) {
    return {
      ...
      template: "<div class=\"mac-tag-autocomplete\"><ul class=\"tag-list\"><li ng-repeat=\"tag in tags\" class=\"tag label\"><div ng-click=\"removeTag(tag)\" class=\"tag-close\">&times;</div><span class=\"tag-label\"></span></li><li class=\"tag input-tag\"><span ng-show=\"disabled\"><input type=\"text\" ng-model=\"textInput\" class=\"text-input no-complete\"/></span><span ng-hide=\"disabled\"><mac-autocomplete mac-autocomplete-url=\"autocompleteUrl\" mac-autocomplete-on-select=\"onSelect(selected)\" mac-autocomplete-on-success=\"onSuccess(data)\" mac-autocomplete-on-key-down=\"onKeyDown(event, value)\" mac-autocomplete-clear-on-select=\"mac-autocomplete-clear-on-select\" class=\"text-input\"></mac-autocomplete></span></li></ul></div>",
      replace: true,
      compile: function(element, attrs) {
        var delay, disabled, events, getSelected, labelKey, queryKey, selectedExp, tagLabelKey, textInput, valueKey;
        ...
        return function($scope, element, attrs) {
          var attrEvent, el, event, eventFn, _fn, _i, _len, _ref;
          $scope.disabled = disabled;
          if ($scope.disabled) {
            _ref = events.split(",");
            _fn = function(eventFn) {
              return $(".no-complete", element).on(event, function($event) {
                var expression;
            ...
            el = $(".no-complete", element).on("keydown", function(event) {
          ...
          $scope.removeTag = function(tag) {
            ...
            return ([].splice.apply($scope.tags, [index, index - index + 1].concat(_ref1 = [])), _ref1);
          };
          ...
          $scope.onSuccess = function(data) {
            ...
            return _(data.data).reject(function(item) {
              var _ref1;
              return _ref1 = item[valueKey] || item, __indexOf.call(existingValues, _ref1) >= 0;
            });
          };
...
angular.module("Mac").directive("macTagInput", [
  "$rootScope", "$parse", function($rootScope, $parse) {
    return {
      ...
      template: "<select multiple=\"multiple\" ng-transclude=\"ng-transclude\"><option ng-repeat=\"item in items\"></option></select>",
      transclude: true,
      replace: true,
          ...
          return chosenElement.change(function(event, object) {
            return $scope.$apply(function() {
              if (object.selected != null) {
                return $scope.selected.push(_($scope.items).find(function(item) {
                  return (item[valueKey] || item) === object.selected;
                }));
              } else if (object.deselected != null) {
                return $scope.selected = _($scope.selected).reject(function(item) {
                  return (item[valueKey] || item) === object.deselected;
                });
              }
            });
          });
...
module.controller("ExampleController", [
  "$scope", function($scope) {
    ...
    $scope.tagAutocompleteOnKeyup = function() {
