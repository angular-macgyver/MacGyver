...
angular.module("Mac").directive("macModal", [
  "$rootScope", "$parse", "modal", "util", "keys", function($rootScope, $parse, modal, util, keys) {
    return {
      restrict: "E",
      template: "<div ng-click=\"closeOverlay($event)\" class=\"modal-overlay hide\"><div class=\"modal\"><a ng-click=\"closeModal($event)\" class=\"close-modal\"></a><div ng-transclude=\"ng-transclude\"></div></div></div>",
      replace: true,
      transclude: true,
      link: function($scope, element, attrs) {
    ...
    return {
      ...
      compile: function(element, attr) {
        ...
        if (attr.reorderableColumns != null) {
          headerSectionElement.find("[table-row]").attr("mac-reorderable", "[mac-cell-template]").attr("mac-reorderable-columns", "");
        }
...
angular.module("Mac").directive("macTagAutocomplete", [
  "$parse", "keys", function($parse, keys) {
    return {
      ...
      compile: function(element, attrs) {
        var attrsObject, delay, events, eventsList, labelKey, queryKey, tagLabelKey, textInput, valueKey;
