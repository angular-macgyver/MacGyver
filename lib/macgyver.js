...
angular.module("Mac").directive("repeatRow", [
    ...
    return {
      ...
      controller: function() {
        ...
        this.repeatCells = function(row, rowElement) {
          var cellMarker, linkerFactory,
            _this = this;
          console.log("CELL TEMPLATES", this.cellTemplates);
          ...
          linkerFactory = function(cell) {
            ...
            console.log("CELL TEMPLATE", templateName, _this.cellTemplates[templateName]);
    ...
    return {
      ...
      compile: function(element, attr, linker) {
        ...
        return function($scope, $element, $attr, controllers) {
          ...
          return controllers[1].cellTemplates[$attr.cell] = [$element, linker, $attr];
        };
      }
    };
  }
    ...
    return {
      ...
      compile: function(element, attr, linker) {
        ...
        return function($scope, $element, $attr, controllers) {
          ...
          return controllers[1].cellTemplates["?"] = [$element, linker, $attr];
        };
      }
    };
  }
  ...
  function() {
    return {
      ...
      compile: function(element, attr) {
        ...
        return function($scope, $element, $attr, controllers) {
          return console.log("test-1: link " + $scope.$id, controllers, $attr);
        };
      }
    };
  }
  ...
  function() {
    return {
      ...
      compile: function(element, attr) {
        ...
        return function($scope, $element, $attrs) {
          return console.log("mac-row-test: linking " + $scope.$id);
        };
      }
    };
  }
  ...
  "$compile", function($compile) {
    return {
      ...
      controller: function() {
        ...
        this.templates = {};
        this.getTemplate = function(columnName) {
          if (!(this.templates[columnName] != null) && (this.templates["?"] != null)) {
            columnName = "?";
          }
          return this.templates[columnName];
        };
      },
      compile: function(element, attr, linker) {
        console.log("mac-row: compile", attr);
        return function($scope, $element, $attr, controllers) {
          var hashKey, rowCtrl, sectionName, tableCtrl, trackExpression, uid;
          ...
          uid = 0;
          hashKey = function(row) {
            var key;
            if (!(key = row.model.$$hashKey)) {
              key = "M" + (++uid);
              row.model.$$hashKey = key;
            }
            return key;
          };
          $scope.$watch(trackExpression, function(rows) {
            var $attrs, block, cell, cellBlock, cellScope, columnName, cursor, key, nextRowMap, row, template, transclude, _i, _j, _len, _len1, _ref;
            if ($scope.lastRowMap == null) {
              $scope.lastRowMap = {};
            }
            if (!rows) {
              return;
            ...
            for (_i = 0, _len = rows.length; _i < _len; _i++) {
              ...
              for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
                ...
                if (!(block.cells[columnName] != null)) {
                  block.cells[columnName] = cellBlock = {};
                  template = rowCtrl.getTemplate(cell.colName);
                  if (!template) {
                    continue;
                  }
                  $element = template[0], transclude = template[1], $attrs = template[2];
                  cellScope = $scope.$new();
                  cellScope.cell = cell;
                  console.log("<---About to clone");
                  block.cells[columnName].element = transclude(cellScope, function(clone) {
                    return clone;
                  });
            ...
            return $scope.lastRowMap = nextRowMap;
          });
        };
      }
    };
    ...
    return {
      ...
      compile: function(element, attrs, transclude) {
        return function($scope, $element, $attrs, controllers) {
          ...
          tableCtrl = controllers[0], rowCtrl = controllers[1];
          return rowCtrl.templates[$attrs.macCellTemplate] = [$element, transclude, $attrs];
        };
      }
    };
  }
    ...
    return {
      ...
      compile: function(element, attrs, transclude) {
        ...
        return function($scope, $element, $attrs, controllers) {
          ...
          tableCtrl = controllers[0], rowCtrl = controllers[1];
          return rowCtrl.templates["?"] = [$element, transclude, $attrs];
        };
      }
    };
  }
  ...
  function() {
    return {
      ...
      link: function(scope, element, attributes) {
        ...
        if (attributes.macUploadEnableOn != null) {
          enableOn = attributes.macUploadEnableOn;
        }
        parent = element.parent();
        input = parent.find("input");
        initialize = function() {
          var options;
          if (((attributes.macUploadDisabled != null) && scope.macUploadDisabled) || !scope.route) {
            return;
          }
          if (isInitialized) {
          ...
          options = {
            ...
            submit: function(event, response) {
              if (attributes.macUploadSubmit != null) {
                return scope.$apply(scope.macUploadSubmit({
                  $event: event,
                  $response: response
                }));
              }
            ...
            success: function(response, status) {
              if (attributes.macUploadSuccess != null) {
                return scope.$apply(scope.macUploadSuccess({
                  $response: response,
                  $data: response.data,
                  $status: status
                }));
        ...
        if (attributes.macUploadSelector != null) {
          element.parents(attributes.macUploadSelector).click(function(event) {
        ...
        scope.$watch("macUploadDisabled", function(isDisabled) {
          if (isDisabled == null) {
            return;
          }
          if (isDisabled) {
        ...
        scope.$watch("macUploadFormData", function(value) {
          if ((value != null) && isInitialized) {
            return input.fileupload("options", {
              formData: value
            });
          }
        });
        if (disableOn != null) {
          scope.$on(disableOn, function() {
            return input.fileupload("disable");
          });
        }
        if (enableOn != null) {
          scope.$on(enableOn, function() {
            return input.fileupload("enable");
          });
        }
...
angular.module("Mac").directive("macColumns", [
  function() {
    return {
      ...
      controller: function() {
        this.trackedColumns = {};
        this.resizeIt = function(scope, element, children, cElement, args) {
          var column, event, percentage, ui, width,
            _this = this;
          ...
          cElement.css("width", "");
          return scope.$apply(function() {
            return _this.recalculateWidths(null, cElement.scope().$id, percentage, +column.width);
          });
        };
        this.reorderIt = function(scope, element, children, cElement, args) {
        ...
        this.getSiblingScopes = function(siblings) {
        ...
        this.recalculateWidths = function(event, id, newValue, oldValue) {
      ...
      link: function(scope, element, attrs, ctrl) {
        ...
        return scope.$on("mac-element-" + scope.$id + "-changed", function() {
          ...
          if (attrs[attributeName] != null) {
            ...
            return callback(scope, element, ctrl.trackedColumns, cElement, args);
          } else if (attributeName === "macResizedCallback") {
            return ctrl.resizeIt(scope, element, ctrl.trackedColumns, cElement, args);
          } else if (attributeName === "macReorderedCallback") {
            return ctrl.reorderIt(scope, element, ctrl.trackedColumns, cElement, args);
          }
        });
      }
    };
  ...
  "Table", function(Table) {
    return {
      ...
      link: function(scope, element, attrs, ctrl) {
        ...
        return scope.$watch("header", function(value) {
          if (!value) {
            return;
          }
          return scope.table.load("header", [scope.header]);
        }, true);
      }
    };
  }
...
angular.module("Mac").directive("macTable2", [
  "$compile", "$filter", "util", function($compile, $filter, util) {
    return {
      ...
      replace: true,
      transclude: true,
      template: "<table class=\"table table-striped mac-table-default\"><tr mac-columns=\"mac-columns\" mac-reorderable=\"&gt; th\"><th ng-repeat=\"cell in table.sections.header.rows[0].cells\" mac-resizable=\"mac-resizable\" mac-column-width=\"{{100/cell.row.cells.length}}\" width=\"{{cell.width}}%\"><a ng-click=\"$parent.predicate=genPredicate(cell.colName);$parent.reverse=!reverse\">{{cell.value()}}</a></th></tr><tr ng-repeat=\"row in table.sections.body.rows | orderBy:predicate:reverse | filter:search\" mac-cells=\"row.cells\"></tr></table>",
      compile: function(element, attrs, transclude) {
...
angular.module("Mac").directive("macTagAutocomplete", [
  "$parse", "$http", "keys", function($parse, $http, keys) {
    return {
      restrict: "E",
      template: "<div class=\"mac-tag-autocomplete\"><ul class=\"tag-list\"><li ng-repeat=\"tag in selected\" class=\"tag label\"><div ng-click=\"selected.splice($index, 1)\" class=\"tag-close\">&times;</div><span class=\"tag-label\"></span></li><li ng-switch=\"disabled\" class=\"tag input-tag\"><span ng-switch-when=\"true\"><input type=\"text\" placeholder=\"{{placeholder}}\" class=\"text-input no-complete\"/></span><span ng-switch-when=\"false\"><mac-autocomplete mac-autocomplete-on-select=\"onSelect(selected)\" mac-autocomplete-on-success=\"onSuccess(data)\" mac-autocomplete-on-key-down=\"onKeyDown(event, value)\" mac-autocomplete-placeholder=\"placeholder\" mac-autocomplete-clear-on-select=\"mac-autocomplete-clear-on-select\" class=\"text-input mac-autocomplete\"></mac-autocomplete></span></li></ul></div>",
      replace: true,
      scope: {
      ...
      compile: function(element, attrs) {
        ...
        return function($scope, element, attrs) {
          ...
          $scope.$watch("disabled", function(value) {
            ...
            if (value) {
              $(".no-complete", element).on("keydown", function(event) {
                return $scope.onKeyDown(event, $(this).val());
              });
            }
            _results = [];
          ...
          $scope.$watch("textInput", function(value) {
            return $(".no-complete", element).val(value);
          });
...
angular.module("Mac").directive("macTime", [
  "util", "keys", "$filter", function(util, keys, $filter) {
    return {
      ...
      replace: true,
      template: "<div class=\"date-time\"><i class=\"mac-icons mac-icon-time\"></i><input type=\"text\" placeholder=\"--:--\" maxlength=\"8\"/></div>",
      compile: function(element, attrs) {
        var defaults, inputElement, opts;
        defaults = {
          id: "time-input",
...
module.controller("ExampleController", [
  "$scope", "$timeout", "Table", function($scope, $timeout, Table) {
    ...
    return $scope.startTime = "04:42 PM";
  }
]);

>>>>>>> Lots of new stuff, splitting column functionality into different directives
    ...
    return {
      ...
      extendAttributes: function(prefix, defaults, attributes) {
        ...
        for (key in defaults) {
          ...
          value = defaults[key];
          macKey = "" + prefix + (this.capitalize(key));
